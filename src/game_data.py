...
if __name__ == "__main__":
    ...
    # Instância de GameData
    game_data = GameData(file_path)
    
    # Carregar dados e exibir resultados básicos
    try:
        game_data.load_data()
        percentages = game_data.calculate_free_paid_percentage()
        most_common_years = game_data.get_year_with_most_releases()
        portuguese_support_count = game_data.count_games_with_portuguese_support()
        
        print("Percentual de jogos gratuitos e pagos:")
        print(f"Gratuitos: {percentages['free_percentage']}%, Pagos: {percentages['paid_percentage']}%")
        
        print("\nAno(s) com o maior número de lançamentos:")
        print(most_common_years)
        
        print("\nNúmero de jogos com suporte ao português:")
        print(portuguese_support_count)
